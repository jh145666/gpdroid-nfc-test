...
public class MainActivity extends Activity implements SEService.CallBack,
        ...
        private ArrayAdapter<String> mSdAdapter;
        ...
        /**
         * Initializes the security domain spinner with known SD AIDs.
         * The first item is "Auto Detect" (null AID), followed by all known
         * security domain AIDs from AID.SD_AIDS.
         */
        public void initSdSpinner() {
                mSdSpinner = (Spinner) findViewById(R.id.sd_spinner);

                List<String> sdNames = new ArrayList<String>();
                sdNames.add(getString(R.string.sd_auto_detect));
                for (String sdName : AID.SD_AIDS.keySet()) {
                        sdNames.add(sdName + " (" + AID.SD_AIDS.get(sdName).toString() + ")");
                }

                mSdAdapter = new ArrayAdapter<String>(this,
                                android.R.layout.simple_spinner_item, sdNames);
                mSdAdapter
                                .setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
                mSdSpinner.setAdapter(mSdAdapter);
                mSdAdapter.notifyDataSetChanged();

                // Set the selected SD AID when the user picks one
                mSdSpinner.setOnItemSelectedListener(new OnItemSelectedListener() {
                        @Override
                        public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                                AID selectedAid = null;
                                if (position > 0) {
                                        // Position 0 = "Auto Detect", positions 1+ = known SD AIDs
                                        String[] sdKeys = AID.SD_AIDS.keySet().toArray(new String[0]);
                                        if (position - 1 < sdKeys.length) {
                                                selectedAid = AID.SD_AIDS.get(sdKeys[position - 1]);
                                        }
                                }
                                GPConnection.getInstance(MainActivity.this).setSelectedSdAID(selectedAid);
                                MAIN_Log.d(LOG_TAG, "Security Domain selected: "
                                                + (selectedAid == null ? "Auto Detect" : selectedAid.toString()));
                        }

                        @Override
                        public void onNothingSelected(AdapterView<?> parent) {
                                // Default to auto-detect
                                GPConnection.getInstance(MainActivity.this).setSelectedSdAID(null);
                        }
                });
        }
